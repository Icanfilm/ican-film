...
// (Note: You can keep this here or use the one in src/components/UI.jsx on GitHub)
...
const Services = ({ navigateTo }) => {
  return (
    <div className="animate-fade-in pt-40 pb-24 selection:bg-red-600 selection:text-white uppercase font-black tracking-tighter">
      <div className="container mx-auto px-6">
        {/* Page Header - Ultra Scale */}
        <div className="text-center max-w-5xl mx-auto mb-40">
          <div className="inline-block bg-red-100 text-red-600 font-black px-8 py-2.5 rounded-full text-[11px] uppercase tracking-[0.6em] mb-10 italic shadow-sm">
          ...
          <p className="text-xl text-slate-600 font-medium leading-relaxed normal-case tracking-normal max-w-3xl mx-auto italic">
            ICAN Film productizes high-end videography into a repeatable monthly system. We don't just "film"—we build the engine that keeps your brand alive across every social platform.
          </p>
        </div>

        {/* The Tiered Packages with Background Imagery and Hover-Pop */}
        <div className="grid lg:grid-cols-3 gap-12 max-w-7xl mx-auto mb-40">
          ...
          <div className="relative group bg-white p-12 rounded-[4rem] shadow-xl border border-slate-100 flex flex-col hover:border-red-600 transition-all transform hover:-translate-y-4 overflow-hidden h-[800px]">
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="bg-red-50 text-red-600 w-24 h-24 rounded-3xl flex items-center justify-center mb-12 group-hover:bg-red-600 group-hover:text-white transition-all shadow-lg">
                <Icon name="lightbulb" size={48} />
              </div>
              <h3 className="text-4xl mb-4 italic leading-none">THE AUDIT</h3>
              ...
              <p className="text-2xl text-slate-500 mb-14 flex-grow font-bold italic normal-case tracking-normal leading-relaxed">
                "Prove the value." A 45-min deep dive where we review your social presence and give you 3-5 tailored content ideas on the spot. No strings attached.
              </p>
              <button onClick={() => navigateTo('booking')} className="w-full py-8 border-4 border-slate-900 text-slate-900 rounded-[2.5rem] hover:bg-slate-900 hover:text-white transition-all text-sm italic shadow-sm uppercase tracking-widest">
                Get Free Audit
              </button>
            </div>
          </div>

          {/* Tier 2: The Flagship Retainer - Ultra Contrast */}
          <div className="relative group bg-slate-900 p-12 rounded-[5rem] shadow-3xl text-white flex flex-col transform lg:scale-110 border-8 border-red-600 overflow-hidden z-20 h-[850px] hover:scale-[1.15] transition-transform duration-700">
            ...
            <div className="relative z-10 flex flex-col h-full pt-10">
              <div className="bg-red-600 text-white w-28 h-28 rounded-[2.5rem] flex items-center justify-center mb-12 shadow-xl shadow-red-600/40 transform group-hover:rotate-12 transition-transform">
                <Icon name="zap" size={64} />
              </div>
              <h3 className="text-6xl mb-4 italic leading-none tracking-tighter">RETAINER</h3>
              <p className="text-red-400 text-[10px] tracking-[0.6em] mb-12 italic leading-none">STARTING $1,500 / MO</p>
              ...
              <button onClick={() => navigateTo('booking')} className="w-full py-10 bg-red-600 text-white rounded-[3rem] hover:bg-red-700 transition-all shadow-3xl text-lg italic uppercase tracking-[0.3em] font-black group">
          ...
          <div className="relative group bg-white p-12 rounded-[4rem] shadow-xl border border-slate-100 flex flex-col hover:border-slate-900 transition-all transform hover:-translate-y-4 overflow-hidden h-[800px]">
            ...
            />
            <div className="relative z-10 flex flex-col h-full">
              <div className="bg-slate-100 text-slate-900 w-20 h-20 rounded-3xl flex items-center justify-center mb-10 group-hover:bg-slate-900 group-hover:text-white transition-all shadow-lg">
                <Icon name="camera" size={40} />
              </div>
              <h3 className="text-4xl mb-4 italic leading-none">EVENTS</h3>
              <p className="text-red-600 text-[10px] tracking-[0.5em] mb-12 italic leading-none uppercase">SELECTIVE SCALE</p>
              <p className="text-xl text-slate-500 mb-14 flex-grow font-bold italic normal-case tracking-normal leading-relaxed">
                Cinematic coverage for galas and industry conferences. Use one event to generate a "Wealth of Content" that lasts your brand 3 months.
              </p>
              <button onClick={() => navigateTo('booking')} className="w-full py-8 border-4 border-slate-900 text-slate-900 rounded-[2.5rem] hover:bg-slate-900 hover:text-white transition-all text-sm italic shadow-sm uppercase tracking-widest">
                Inquire Project
              </button>
        ...
        <div className="bg-slate-50 rounded-[5rem] p-24 text-center border border-slate-100">
           ...
           <p className="text-2xl text-slate-500 font-bold max-w-3xl mx-auto normal-case italic mb-20 leading-relaxed">
             "Outsourcing content creation frees up owners' time for higher-level tasks." By productizing our service, we deliver 300% more reach across platforms for a fraction of the cost of an internal team.
           </p>
           <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12">
              {[
                ...
                <div key={i} className="bg-white p-12 rounded-[3.5rem] shadow-sm group hover:bg-slate-900 hover:text-white transition-all duration-500">
                   <p className="text-red-600 text-xs font-black tracking-widest mb-4 group-hover:text-red-400">{stat.value}</p>
                   <p className="text-2xl font-black tracking-tighter uppercase leading-none italic">{stat.label}</p>
                </div>
              ))}
           </div>
        </div>
